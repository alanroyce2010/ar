...
def find_homography_aruco_with_cache(image, aruco_marker, sigs):
    global CACHED_SRC_PTS, CACHED_DST_PTS
    found = False
    ...
    if found:
        ...
        if H is None and CACHED_DST_PTS is not None and CACHED_SRC_PTS is not None:
            H, mask = cv2.findHomography(CACHED_SRC_PTS.reshape(-1, 1, 2), CACHED_DST_PTS.reshape(-1, 1, 2), cv2.RANSAC, 5.0)
            return True, H
        elif H is None:
            return False, None
        else:
...
def find_pattern_aruco(image, aruco_marker, sigs):
    global CACHED_DST_PTS, CACHED_SRC_PTS
        ...
        if approx.shape[0]==4:
            ...
            if norm > 100:
                ...
                if (match1 or match2 or match3 or match4):
                    ...
                    if useCache:
                        CACHED_SRC_PTS = src_pts
                        CACHED_DST_PTS = dst_pts
